...
class FormEditarCargo(FormBase):
    ...
    usuarios = QuerySelectMultipleField('Usuários', query_factory=lambda: Usuario.query.all(),
                                                    allow_blank=True)
...
class FormCriarCampus(FormBase):
    ...
    instituicao = QuerySelectField('Instituição', query_factory=lambda: Instituicao.query.all())

    mapeamento = GeoJSONField('Mapeamento', srid=-1, session=db.session,
                              geometry_type= 'MULTIPOLYGON',
                              render_kw={'data-width':400, 'data-height':400, 'data-zoom':10,
...
class FormCriarBloco(FormBase):
    ...
    departamento = QuerySelectField('Departamento', query_factory=lambda: Departamento.query.all())
...
class FormCriarAmbienteInterno(FormBase):
    ...
    andar = Select2Field('Andar',
      choices=[('Térreo', 'Térreo')] + [(str(n)+'º Andar', str(n)+'º Andar') for n in range(1, 11)])
    ...
    area = DecimalField('Área', places=2, validators=[NumberRange(0)])
...
class FormEditarAmbienteInterno(FormBase):
    ...
    equipamentos = QuerySelectMultipleField('Equipamentos', query_factory=lambda: Equipamento.query.all(),
                                                            allow_blank=True)
...
class FormEditarSubestacaoAbrigada(FormBase):
    ...
    localizacao = GeoJSONField('Localização', srid=-1, session=db.session, geometry_type= 'POINT',
                               render_kw={'data-width':400, 'data-height':400})
...
class FormCriarExtintor(FormBase):
    ...
    carga_nominal = DecimalField('Carga Nominal (kg)', places=2, validators=[NumberRange(0)])
    ...
    intervalo_manutencao = IntegerField('Intervalo de Manutenção', validators=[InputRequired(),
                                                                               NumberRange(0)])
...
class FormEditarExtintor(FormBase):
    ...
    manutencoes = QuerySelectMultipleField('Manutenções', query_factory=lambda: Manutencao.query.all(),
                                                          allow_blank=True)
